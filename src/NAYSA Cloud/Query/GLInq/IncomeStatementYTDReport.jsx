          ...
          <div
            ...
            className={
              mobileAnimDirection === "back"
                ? "is-mobile-slide-back"
                : "is-mobile-slide-forward"
            }
          >
            {mobileView === "card" ? (
              <div className="space-y-2">
                {currentMobileLevelNodes.map((node) => (
                  <IncomeStatementNodeRowMobileCard
                    key={node.id}
                    node={node}
                    periods={safePeriods}
              ...
              <IncomeStatementMobileTableView
...
function IncomeStatementNodeRowDesktop({
  node,
  level,
  periods,
  gridTemplateColumns,
  ...
  return (
    <>
      ...
      {hasChildren && isExpanded ? (
        <div className="is-desktop-expand">
          {node.children.map((child) => (
            <IncomeStatementNodeRowDesktop
              key={child.id}
              node={child}
...
function IncomeStatementNodeRowMobileCard({
  node,
  periods,
  onOpenNode,
  onJumpToGLInquiry,
...
function IncomeStatementMobileTableView({
  nodes,
  periods,
  onOpenNode,
  onJumpToGLInquiry,
...
function ISAnimationStyles() {
  return (
    <style>{`
      @keyframes isDesktopExpand {
        from {
          opacity: 0;
      ...
      @keyframes isMobileSlideForward {
        from {
          opacity: 0;
      ...
      .is-desktop-expand {
        animation: isDesktopExpand 180ms ease-out;
      }

      .is-mobile-slide-forward {
        animation: isMobileSlideForward 220ms ease-out;
        will-change: transform, opacity;
      }

      .is-mobile-slide-back {
        animation: isMobileSlideBack 220ms ease-out;
        will-change: transform, opacity;
      }
