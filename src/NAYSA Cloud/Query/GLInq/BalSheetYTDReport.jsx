  ...
  return (
    <div className="w-full overflow-hidden rounded-xl border border-slate-200 bg-white">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="grid grid-cols-2 gap-2 md:flex md:items-center">
            <button
              type="button"
              onClick={expandAll}
              className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-medium text-gray-700 transition hover:bg-slate-100"
            >
              Expand All
            </button>
            ...
            <button
              type="button"
              onClick={collapseAll}
              className="rounded-md border border-slate-200 bg-white px-3 py-2 text-[11px] font-medium text-gray-700 transition hover:bg-slate-50"
            >
              Collapse All
            </button>
          </div>
      ...
      <div className="block md:hidden">
        <div className="max-h-[calc(100vh-240px)] overflow-y-auto bg-slate-50/50 p-3">
          <div className="space-y-2">
            {treeData.map((node) => (
              <BalanceSheetNodeRowMobile
                key={node.id}
                node={node}
                level={0}
                periods={safePeriods}
...
function BalanceSheetNodeRowMobile({
  node,
  level,
  periods,
  expandedMap,
  ...
  const leftIndent = Math.min(level * 12, 32);

  return (
    <>
      <div
        className={`overflow-hidden rounded-xl border shadow-sm ${containerClass}`}
          ...
          <div className="flex items-start gap-2">
            {hasChildren ? (
              <button
                type="button"
                onClick={() => onToggle(node.id)}
                className="mt-[2px] flex h-7 w-7 shrink-0 items-center justify-center rounded-md border border-slate-200 bg-white text-gray-500 hover:bg-slate-100"
              >
                <FontAwesomeIcon
                  icon={isExpanded ? faChevronDown : faChevronRight}
                  className="text-[10px]"
              ...
              <div className="mt-[2px] flex h-7 w-7 shrink-0 items-center justify-center text-slate-300">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-start justify-between gap-2">
                ...
                <div
                  className={`inline-flex shrink-0 items-center gap-1 rounded-full border px-2 py-1 text-[10px] font-semibold ${typeMeta.badgeClass}`}
                >
                  <FontAwesomeIcon icon={typeMeta.icon} className={typeMeta.iconClass} />
                  <span>{typeMeta.label}</span>
                </div>
              ...
              {(canJumpToGL || canJumpToRC || canJumpToSL) && (
                <div className="mt-2">
                  <button
                    type="button"
                    onClick={handleRowJump}
                    className="inline-flex items-center gap-1 rounded-md border border-blue-200 bg-blue-50 px-2.5 py-1.5 text-[10px] font-semibold text-blue-700 hover:bg-blue-100"
                  >
                    <FontAwesomeIcon
                      icon={faArrowUpRightFromSquare}
                      className="text-[9px]"
              ...
              <div className="mt-3 space-y-1.5">
                {periods.map((periodCode) => {
                  ...
                  return (
                    <div
                      key={`${node.id}-${periodCode}`}
                      className="flex items-center justify-between gap-3 rounded-md bg-slate-50 px-2.5 py-2"
                    >
                      ...
                      <div className={`text-right tabular-nums ${getAmountClass(displayAmount, node.rowType)}`}>
                        {formatNumberDisplay(displayAmount)}
                      </div>
                    </div>
                  );
...
function getAmountClass(value, rowType) {
  ...
  return "text-[11px] font-medium text-gray-400";
}
