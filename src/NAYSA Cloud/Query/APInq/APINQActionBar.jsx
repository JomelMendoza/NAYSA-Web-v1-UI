// src/AccountsReceivable/APINQ/APINQActionBar.jsx
import React, { useRef, useEffect, useState } from "react";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import {
    faSave, faUndo, faPrint, faFileExport, faMagnifyingGlass,
    faRedo, faFilePdf, faInfoCircle, faSpinner, faDatabase
 ...
 * - busy?: boolean
 */
export default function APINQActionBar({ activeTab, onAction, help = {}, busy = false }) {
    const [openGuide, setOpenGuide] = useState(false);
    const ref = useRef(null);

    const click = (id) => () => onAction?.(id);

    // Button map per tab (IDs are used by the handler in APINQ.jsx)
    const BUTTONS = {
        inquiry: [
