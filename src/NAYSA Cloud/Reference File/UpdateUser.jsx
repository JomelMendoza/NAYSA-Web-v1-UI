...
const UpdateUser = () => {
    ...
    const handleEditUser = async (user) => {
        ...
        setPosition(user.position || "");
        setEmailAdd(user.emailAdd || "");
        setActive(user.active === "Y" ? "Yes" : user.active === "P" ? "Pending" : "No");
        setViewCostamt(user.viewCostamt === "Y" ? "Y" : "N");
        setEditUprice(user.editUprice === "Y" ? "Y" : "N");
    ...
    const handleExport = (format) => {
        ...
        try {
            const payload = {
                json_data: {
                    filter: activeTab === "active" ? "Active" : activeTab === "pending" ? "Pending" : "Inactive",
                },
                format,
            };

            apiClient.post("/users/export", payload, { responseType: "blob" }).then((response) => {
                const url = window.URL.createObjectURL(new Blob([response.data]));
                const link = document.createElement("a");
                link.href = url;
                const fileName = `users_export_${format}_${new Date().toISOString().slice(0, 10)}.${format}`;
            ...
            <div className="global-tran-tab-div-ui">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="global-ref-textbox-group-div-ui">
                        <div className="relative">
                            <input
                                ...
                                disabled={isEditing && selectedUser}
                                className={`peer global-ref-textbox-ui ${isEditing ? "global-ref-textbox-enabled" : "global-ref-textbox-disabled"
                                    } ${isEditing && selectedUser ? "bg-blue-100 cursor-not-allowed" : ""}`}
                            />
                            <label
                                htmlFor="userId"
                    ...
                    <div className="global-ref-textbox-group-div-ui">
                        <div className="relative">
                            <div
                                className={`flex items-center global-ref-textbox-ui ${isEditing ? "global-ref-textbox-enabled cursor-pointer" : "global-ref-textbox-disabled"
                                    }`}
                                onClick={handleOpenBranchModal}
                            >
                                <input
