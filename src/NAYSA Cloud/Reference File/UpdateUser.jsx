  ...
  const handleCheckDuplicate = async () => {
    ...
    try {
      ...
      if (String(parsed?.result) === "1") {
        await useSwalErrorAlert("Duplicate", "User ID already exists. Please use a different ID.");
        setUserId("");
        setTimeout(() => {
  ...
  const tableRows = useMemo(() => {
    return users
      ...
      .map((u) => ({
        ...u,
        activeLabel: activeLabel(u.active),
        // Branch: prefer name, fall back to code
        branchDisplay: u.branchName || u.branchCode || "-",
        // Department: prefer name, fall back to code
        rcDisplay: u.rcName || u.rcCode || "-",
        // User type: map code to full label
