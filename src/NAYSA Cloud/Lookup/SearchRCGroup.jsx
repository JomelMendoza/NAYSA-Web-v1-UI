...
const SearchRCGroup = ({
  ...
  onClose,
  currentRcCode,
  title = "Select RC Group Code",
  withPagination = false, // Defaults to false to match original behavior, but supports pagination
}) => {
  ...
  // Fetching from /rcMast because /lookupRCMast excludes rcGroup='Y'
  const {
    data: fullRcList = [],
    isLoading,
    isFetching,
  ...
  const filteredAndSorted = useMemo(() => {
    if (!fullRcList.length) return [];

    let result = fullRcList.filter((item) => {
      const isGroup = item.rcGroup === "Y";
      const isNotSelf = item.rcCode !== currentRcCode;
      const matchesCode = (item.rcCode || "")
        .toLowerCase()
        .includes(debouncedFilters.rcCode.toLowerCase());
      const matchesName = (item.rcName || "")
        .toLowerCase()
        .includes(debouncedFilters.rcName.toLowerCase());

      return isGroup && isNotSelf && matchesCode && matchesName;
    });
    ...
    return result;
  }, [fullRcList, debouncedFilters, sortConfig, currentRcCode]);
  ...
  const handleApply = (selectedCode) => {
    onClose(selectedCode);
    setFilters({ rcCode: "", rcName: "" }); // Reset filters on close
  };
        ...
        <div className="flex-grow overflow-auto custom-scrollbar bg-white">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center h-64 text-slate-400">
              ...
              <p className="text-sm font-medium">Fetching Groups...</p>
            </div>
          ) : isError ? (
                ...
                {paginatedData.length > 0 ? (
                  paginatedData.map((rcItem, index) => (
                    <tr
                      ...
                      onClick={() => handleApply(rcItem.rcCode)}
                      className="group hover:bg-blue-50 cursor-pointer transition-colors"
                    >
                      <td className="global-lookup-td-ui w-[180px] font-bold">{rcItem.rcCode}</td>
                      <td className="global-lookup-td-ui w-[300px]">{rcItem.rcName}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
