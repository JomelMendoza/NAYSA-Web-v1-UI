...
const AreaLookupModal = ({ isOpen, onClose }) => {
  ...
  const {
    ...
    queryKey: ["lookupArea"],
    queryFn: async () => {
      const { data: result } = await apiClient.get("/lookupArea", {
        params: {
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    let result = areas.filter((item) => {
      const code = String(item?.areaCode ?? "").toLowerCase();
      const name = String(item?.areaName ?? "").toLowerCase();

      return (
        code.includes(debouncedFilters.areaCode.toLowerCase()) &&
        name.includes(debouncedFilters.areaName.toLowerCase())
      );
    });
    ...
    return result;
  }, [areas, debouncedFilters, sortConfig]);
...
export default AreaLookupModal;