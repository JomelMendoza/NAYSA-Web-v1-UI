...
export default function BillTermRef() {
  ...
  const checkInUsed = async (code) => {
    ...
    const res = await apiClient.post("/checkInUsedBillterm", {
      json_data: { billtermCode: c },
    });
  ...
  const handleBillTermCodeBlur = async (e) => {
    ...
    if (e?.type === "keydown") {
      if (e.key !== "Enter") return;
      e.preventDefault();
      e.target.dataset.enterValidated = "1";
    }
    ...
    try {
      ...
      if (dup) {
        await useSwalErrorAlert(
          "Duplicate Entry",
          "Duplicate Billing Term Code is not allowed."
        );
        updateForm({ code: "" });
        setIsDupCode(false);
        setTimeout(() => codeInputRef.current?.focus?.(), 0);
  ...
  const loadList = useCallback(async () => {
    try {
      ...
      console.error(err);
      await useSwalErrorAlert("Error", "Failed to load Billing Terms.");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const save = async () => {
    ...
    const payload = {
      json_data: {
        billtermCode: String(f.code ?? "").trim().toUpperCase(),
        billtermName: String(f.name ?? "").trim(),
        dueDays: String(f.daysDue ?? "").trim() === "" ? null : Number(f.daysDue),
        userCode,
      },
    ...
    try {
      ...
      if (Number(resp?.data?.errorcount || 0) > 0 || resp?.data?.success === false) {
        await useSwalErrorAlert(
          "Validation Error",
          resp?.data?.errormsg || resp?.data?.message || "Validation error."
        );
        return;
      }
      ...
      await useSwalSuccessAlert(
        "Success!",
        "Billing Term saved successfully."
      );
    ...
    async (row) => {
      ...
      if (used) {
        return useSwalErrorAlert(
          "Cannot Delete",
          `Billing Term "${code}" is already in use.`
        );
      }
      ...
      try {
        const resp = await apiClient.post("/deleteBillterm", {
          json_data: {
            billtermCode: code,
            userCode,
          },
        ...
        if (resp?.data?.success === false) {
          await useSwalErrorAlert(
            "Error",
            resp?.data?.message || "Failed to delete record."
          );
          return;
        }
        ...
        await useSwalDeleteRecord(
          "Deleted!",
          `Billing Term "${code}" has been successfully removed.`
        );

        setRows((prev) => prev.filter((r) => r.code !== code));
        setAllRows((prev) => prev.filter((r) => r.code !== code));
  ...
  const columns = useMemo(
    () => [
      { key: "code", label: "Billing Term Code", sortable: true, width: 180 },
      { key: "name", label: "Billing Term Name", sortable: true, width: 360 },
      {
        key: "daysDue",
        label: "Due Days",
        sortable: true,
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
        <Card>
          ...
          <FieldRenderer
            ...
            type="number"
            value={form.daysDue}
            onChange={(e) => updateForm({ daysDue: e?.target?.value || "" })}
            disabled={!isEditing}
          />
